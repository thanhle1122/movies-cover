...
<body>
    ...
    <main role="main" class="mb-2">
        ...
        <div class="container mt-2">
            <div class="full borderf">
                <h1 class="tieu-de">Quản lý Quốc gia Phim</h1>
            </div>
            <?php
        ...
        <div class="container">
            <div class="full borderf">
                <h3 class="text-movies">Quốc gia Phim</h3>
            </div>
            <br>
            <div class="row">
                <div class="col">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody class="danhsach">
                            ...
                            $result_count = mysqli_query($conn, "SELECT COUNT(*) As total_records FROM countries");
                            $total_records = mysqli_fetch_array($result_count);
                            $total_records = $total_records['total_records'];
                            $total_no_of_pages = ceil($total_records / $total_records_per_page);
                            $second_last = $total_no_of_pages - 1; // total page minus 1

                            $result = mysqli_query($conn, "SELECT * FROM countries LIMIT $offset, $total_records_per_page");
                            $dataQuocGia = [];
                            ...
                            // var_dump($dataQuocGia); die;
                            ?>
                            <?php foreach ($dataQuocGia as $item) : ?>
                                <tr>
                                    <td style="text-align: center;"><?= $item['countries_id'] ?></td>
                                    <td><?= $item['countries_title'] ?></td>
                                    <td><?= $item['countries_description'] ?></td>
                                    <td style="text-align: center;">
                                        <?php if ($item['countries_status'] == 1) : ?>
                                            <?= $item['countries_status'] = "Đã duyệt" ?>
                                        <?php else : ?>
                                            <?= $item['countries_status'] = "Chưa duyệt" ?>
                                        <?php endif; ?>
                                    </td>
                                    <td style="display: flex;justify-content: center;">
                                        <div style="display: flex;">
                                            <a title="Chỉnh sửa" href="update.php?countries_id=<?= $item['countries_id'] ?>" class="btn btn-warning">
                                                <i class="material-icons">edit</i>
                                            </a>
                                            <div style="border:1px #ddd none;height:10px;width:10px "></div>
                                            <button title="Xóa" type="button" class="btn btn-danger btnDelete" data-countries_id="<?= $item['countries_id'] ?>">
    ...
    <script>
        $('.btnDelete').on('click', function(e) {
            // Lấy giá trị của thuộc tính "data-countries_id" của nút mà người dùng đang click
            var countries_id = $(this).attr('data-countries_id');

            // Hiển thị cảnh báo
            var xacNhanXoa = confirm('Bạn có chắc chắn muốn xóa?');
            if (xacNhanXoa == true) { // Người dùng đã chọn Yes
                // Điều hướng đến trang delete.php với tham số countries_id được truyền theo request GET
                location.href = 'delete.php?countries_id=' + countries_id;
            }
        });
    </script>
