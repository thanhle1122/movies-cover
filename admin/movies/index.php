...
<body>
    ...
    <main role="main" class="mb-2">
        ...
        <div class="container mt-2">
            <div class="full borderf">
                <h1 class="tieu-de">Quản lý Danh sách Phim</h1>
            </div>
            <?php
        ...
        <div class="container">
            <div class="full borderf">
                <h3 class="text-movies">Danh sách Phim</h3>
            </div>
            <br>
            <div class="row">
                <div class="col">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody class="danhsach">
                            ...
                            $result_count = mysqli_query($conn, "SELECT COUNT(*) As total_records FROM movies");
                            $total_records = mysqli_fetch_array($result_count);
                            $total_records = $total_records['total_records'];
                            $total_no_of_pages = ceil($total_records / $total_records_per_page);
                            $second_last = $total_no_of_pages - 1; // total page minus 1

                            $result = mysqli_query($conn, "SELECT * FROM movies LIMIT $offset, $total_records_per_page");
                            $dataPhim = [];
                            ...
                            ?>
                            <?php foreach ($dataPhim as $item) : ?>
                                <tr>
                                    <td><?= $item['movies_id'] ?></td>
                                    <td><?= $item['movies_title'] ?></td>
                                    <td><?= $item['movies_description'] ?></td>
                                    <td>
                                        <?php if ($item['movies_status'] == 1) : ?>
                                            <?= $item['movies_status'] = "Đang tiến hành" ?>
                                        <?php else : ?>
                                            <?= $item['movies_status'] = "Hoàn thành" ?>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <img src="/web-xem-phim/assets/images/<?= $item['movies_image'] ?>" class="img-fluid" alt="">
                                    </td>
                                    <td style="text-align: center;"><?= $item['movies_year'] ?></td>
                                    <td style="text-align: center;"><?= $item['movies_list_episodes'] ?></td>
                                    <td>
                                        <?php if ($item['movies_category_id'] == 1) : ?>
                                            <?= $item['movies_category_id'] = "Phim đề cử" ?>
                                        <?php elseif ($item['movies_category_id'] == 2) : ?>
                                            <?= $item['movies_category_id'] = "Phim mới cập nhật" ?>
                                        <?php else : ?>
                                            <?= $item['movies_category_id'] = "Phim phổ biến" ?>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <?php if ($item['movies_genre_id'] == 1) : ?>
                                            <?= $item['movies_genre_id'] = "Anime" ?>
                                        <?php elseif ($item['movies_genre_id'] == 34) : ?>
                                            <?= $item['movies_genre_id'] = "CN Animation" ?>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <?php if ($item['movies_country_id'] == 1) : ?>
                                            <?= $item['movies_country_id'] = "Nhật Bản" ?>
                                        <?php else : ?>
                                            <?= $item['movies_country_id'] = "Trung Quốc" ?>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <div style="display: flex;">
                                            <a title="Chỉnh sửa" href="update.php?movies_id=<?= $item['movies_id'] ?>" class="btn btn-warning">
                                                <i class="material-icons">edit</i>
                                            </a>
                                            <div style="border:1px #ddd none;height:10px;width:10px "></div>
                                            <button title="Xóa" type="button" class="btn btn-danger btnDelete" data-movies_id="<?= $item['movies_id'] ?>">
    ...
    <script>
        $('.btnDelete').on('click', function(e) {
            // Lấy giá trị của thuộc tính "data-movies_id" của nút mà người dùng đang click
            ...
            if (xacNhanXoa == true) { // Người dùng đã chọn Yes
                // Điều hướng đến trang delete.php với tham số movies_id được truyền theo request GET
                location.href = 'delete.php?movies_id=' + movies_id;
            }
        });
    </script>
